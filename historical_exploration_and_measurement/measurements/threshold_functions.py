...
def single_fit_give(xTest,yTest,xData,yData,zData,**kwargs):
    
    initialFitReturn = SmoothBivariateSpline(xData,yData,zData,s=kwargs['s']).ev(xTest,yTest)
    ...
    posFitReturn = SmoothBivariateSpline(adjXData,adjYData,posAdjZData,s=kwargs['s']).ev(xTest,yTest)
    negFitReturn = SmoothBivariateSpline(adjXData,adjYData,negAdjZData,s=kwargs['s']).ev(xTest,yTest)
...
def fit_give(xTest,yTest,xData,yData,zData,**kwargs):
    
    dim = np.core.fromnumeric.shape(xTest)
    
    if np.size(dim) == 0:
        give = single_fit_give(xTest,yTest,xData,yData,zData,s=kwargs['s'])
    ...
    if np.size(dim) == 1:
        for i in range(dim[0]):
            give[i] = single_fit_give(xTest[i],yTest[i],xData,yData,zData,s=kwargs['s'])
    ...
    for i in range(dim[0]):
        for j in range (dim[1]):
            give[i,j] = single_fit_give(xTest[i,j],yTest[i,j],xData,yData,zData,s=kwargs['s'])
