        ...
        for i in range(self.numSectors):
            ...
            if type(self.intersectionSet[i]) is geo.point.Point:
                ...
                if self.centreIsWithin:
                    ...
                    self.dist[i] = self.sectorMidline[i].project(self.intersectionSet[i])                    
                    self.sectorFactors[i] = self.circle_fit(self.dist[i])
                    
                    if self.dist[i] < self.minDistance and not(self.ignoreErrors):
                ...
                if (np.mod(len(self.intersectionSet[i]),2) == 1 ) & (self.centreIsWithin):
                    ...
                    self.sectorFactors[i] = (sum(self.circle_fit(self.dist[i][::2])) - 
                    ...
                    self.sectorFactors[i] = (sum(self.circle_fit(self.dist[i][1::2])) - 
                ...
                if self.dist[i][0] < self.minDistance and not(self.ignoreErrors):
                    raise Exception("Centre too close to edge")                    
