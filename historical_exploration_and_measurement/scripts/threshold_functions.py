...
def single_angle_gap(xTest,yTest,xData,yData,xScale,yScale):
    ...
    ref = (dx == 0) & (dy == 0)
    dx = np.delete(dx,np.where(ref)[0])
    dy = np.delete(dy,np.where(ref)[0])
...
def single_fit_give(xTest,yTest,xData,yData,zData,w):
    ...
    initialFitReturn = SmoothBivariateSpline(xData,yData,zData,w=w).ev(xTest,yTest)
    ...
    posFitReturn = SmoothBivariateSpline(adjXData,adjYData,posAdjZData,w=wAdj).ev(xTest,yTest)
    negFitReturn = SmoothBivariateSpline(adjXData,adjYData,negAdjZData,w=wAdj).ev(xTest,yTest)
...
def fit_give(xTest,yTest,xData,yData,zData,w):
    
    dim = np.core.fromnumeric.shape(xTest)
    
    if np.size(dim) == 0:
        give = single_fit_give(xTest,yTest,xData,yData,zData,w)
    ...
    if np.size(dim) == 1:
        for i in range(dim[0]):
            give[i] = single_fit_give(xTest[i],yTest[i],xData,yData,zData,w)
    ...
    for i in range(dim[0]):
        for j in range (dim[1]):
            give[i,j] = single_fit_give(xTest[i,j],yTest[i,j],xData,yData,zData,w)
